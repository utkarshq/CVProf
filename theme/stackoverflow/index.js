...
function render(resume) {
  if (resume.meta && resume.meta.locale) {
    // Load locale if needed, moment usually has en built-in
    if (resume.meta.locale !== 'en') {
      try {
        require(`moment/locale/${resume.meta.locale}`);
        moment.locale(resume.meta.locale);
      } catch (e) {
        console.error(`Could not load locale ${resume.meta.locale}:`, e);
      }
    } else {
      moment.locale('en');
    }
